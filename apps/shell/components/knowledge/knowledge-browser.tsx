...
export function KnowledgeBrowser({
  ...
  initialEntries: KnowledgeEntry[] | null;
}) {
  const { user } = useAuth();
  // SHAN-509: the page is server-rendered now, so the delete X on each card and
  // the Edit button's enabled state are prerendered as signed-out — the server
  // cannot know who is asking. Gating on `useAuth().user` alone would render
  // them during this component's own hydration pass and throw React #418
  // (SHAN-492); `useHydrated` is false until the commit after that.
  const hydrated = useHydrated();
  const currentUserId = hydrated ? user?.id ?? null : null;
  ...
  // SHAN-509: the filters deliberately do NOT seed from the URL any more.
  // SHAN-499 could read `window.location` straight into state because the first
  // paint was the skeleton on both sides of hydration, so no markup depended on
  // it. Now that the server renders the real list, the search box's value, the
  // active CategoryTabs pill, the location <select> and the EntryDetail panel
  // all depend on a query string the server never saw. The URL is applied in a
  // post-mount effect below instead, one commit later.
  ...
  const hasLoaded = useRef(initialEntries !== null);
  // Suppresses only the FIRST run of the load effect, and only when the server
  // already seeded the same unfiltered list the mount fetch would have asked
  // for. Every later run — a tab click, a search, the URL applied below — must
  // still fetch.
  const skipInitialLoad = useRef(initialEntries !== null);
  ...
  // list the server prerendered. A plain /knowledge load applies EMPTY_FILTER,
  // which sets every filter to the value it already holds, so React bails out
  // and no extra request goes out. A ?category= link re-runs the load effect
  // above exactly once — the mount fetch it would otherwise have raced was
  // already skipped by `skipInitialLoad`.
  useEffect(() => {
