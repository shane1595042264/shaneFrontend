...
interface VocabularyBrowserProps {
  /**
   * The unfiltered first page of words, rendered on the server so the document
   * carries real content (SHAN-507). `null` means the server fetch failed; the
   * browser then falls back to the original client-fetch-on-mount path.
   */
  initialWords: VocabWord[] | null;
}

export function VocabularyBrowser({ initialWords }: VocabularyBrowserProps) {
  ...
  const hasLoaded = useRef(initialWords !== null);
  // Suppresses only the first run of the load effect below when the server
  // already seeded `words`. Filter/search changes rebuild loadWords and fetch
  // as they always have.
  const skipInitialLoad = useRef(initialWords !== null);
