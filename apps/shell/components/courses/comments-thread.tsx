...
// Adapted copy of components/knowledge/comments-thread.tsx (SHAN-437):
...
export function CourseCommentsThread({ courseId, courseOwnerId }: Props) {
  ...
  const refresh = async () => {
    setLoading(true);
    try {
      const list = await listCourseComments(courseId);
      setComments(list);
    } catch (e: unknown) {
  ...
  const submit = async () => {
    ...
    setError(null);
    try {
      await postCourseComment(courseId, text.trim(), replyTo ?? undefined);
      setText("");
      setReplyTo(null);
      await refresh();
    } catch (e: unknown) {
  ...
  const startEdit = (c: CourseComment) => {
    setEditError(null);
    setEditingId(c.id);
  ...
  const saveEdit = async () => {
    ...
    setEditError(null);
    try {
      await editCourseComment(editingId, editText.trim());
      setEditingId(null);
      setEditText("");
      await refresh();
    } catch (e: unknown) {
  ...
  const renderComment = (c: CourseComment, isReply: boolean) => {
    const canDelete =
      !!user && (user.id === c.authorId || (courseOwnerId !== null && user.id === courseOwnerId));
    ...
    <section className="mt-6 border-t border-white/10 pt-4">
      ...
      {deleteConfirmId && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="course-comment-delete-heading"
        >
          <FocusTrappedDiv
          ...
          >
            <h3
              id="course-comment-delete-heading"
