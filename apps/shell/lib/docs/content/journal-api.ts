...
The two If-Match writes (revert, suggestion approve) take the version number as a plain integer, seeded from \`currentVersionNum\` on \`GET /entries/:date\`. **From a browser, send it as \`X-If-Match\`** (SHAN-489): shanejli.com requests are same-origin and ride a rewrite through Vercel's edge, which evaluates a real \`If-Match\` against the response ETag. Ours are weak validators, a weak validator can never satisfy \`If-Match\`'s strong comparison, and the edge therefore turns the origin's 200 into a 412 *after* the write has committed: a success reported as a failure, and a retry that appends the revert twice. \`If-Match\` stays correct for direct callers hitting Railway with a PAT, and wins if both are sent. Full writeup in [Conventions](/docs/conventions).
